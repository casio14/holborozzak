...
- `scripts/` — üres. Az automatikus esemény-gyűjtő workflow-kat (napi `collect.yml` +
  ...
  nem használt. **`/esemeny-gyujtes` Claude Code skill** (`.claude/skills/esemeny-gyujtes/`,
  CSAK kézi indítás, se token, se API-kulcs): a `docs/esemeny-forrasok.md` honlaplistáját
  végigjárva kigyűjti az új eseményeket a **`public/admin/jelolt-import.json`** fájlba
  (commit + push → auto-deploy), amit az admin → Jelöltek oldal megnyitáskor automatikusan
  beimportál (dedup miatt idempotens, a fájl a repóban marad, futásonként felülírva).
